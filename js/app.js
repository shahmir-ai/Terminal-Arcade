...
createHallway() {
    ...
    const elevatorLight = new THREE.PointLight(0xFFFFFF, 1.5, 4); // Brighter light with longer range
    ...
    const lightGeometry = new THREE.BoxGeometry(0.5, 0.1, 1.8); // Increased length from 1.0 to 1.8
    ...
    // Add 5 ceiling lights along the hallway (reduced from 9 to improve performance)
    for (let i = 0; i < 5; i++) {
        const light = new THREE.Mesh(lightGeometry, lightMaterial);
        // Adjusted spacing to distribute 5 lights evenly (approximately 6 units apart)
        light.position.set(doorPositionX, hallwayHeight - 0.1, doorPositionZ + 6 + i * 4);
        this.hallway.add(light);
        
        // Add point light at each ceiling light with increased range and intensity
        const pointLight = new THREE.PointLight(0xFFFFFF, 0.7, 6); // Increased intensity and range
        pointLight.position.set(doorPositionX, hallwayHeight - 0.2, doorPositionZ + 3 + i * 6);
...
// Check if this is the return portal
    ...
    foundInteractive = true;
                ...
                // Check if this is the elevator (only handle in arcade level)
                else if (currentObject.userData && currentObject.userData.isElevator && this.currentLevel === 'arcade') {
                    ...
                    this.isElevatorActivated = true;
                    
                    // Log the elevator interaction
                    console.log('Elevator sound played, controls disabled');
                    
                    // Transition to corridor level after sound plays
                    setTimeout(() => {
                        this.transitionToLevel('corridor');
                    }, 2000); // Wait 2 seconds for the sound to finish
...
initLevelManager() {
    ...
    this.levelManager = {
        ...
        loadCorridorLevel: async () => {
            console.log('Loading corridor level...');
            
            // Check if corridor.js is already loaded
            if (typeof window.CorridorLevel === 'undefined') {
                // Load the corridor.js script
                await this.loadScript('js/corridor.js');
            }
            
            // Create the corridor level instance
            if (!this.levelManager.corridorLevel) {
                this.levelManager.corridorLevel = new CorridorLevel(this);
            }
            
            return this.levelManager.corridorLevel;
        },
        
        // Unload the arcade level
        unloadArcadeLevel: () => {
            console.log('Unloading arcade level...');
            
            // Remove all arcade-specific objects from the scene
            // Keep only essential objects like camera, lights, etc.
            ...
            // Clear references to arcade room objects
            this.arcadeRoom = {
                floor: null,
                walls: []
            };
            ...
            console.log(`Removed ${objectsToRemove.length} objects from the scene`);
        }
    };
}
...
/**
 * Fade the transition overlay to black
 * @param {number} duration - Duration of the fade in milliseconds
 * @returns {Promise} - Promise that resolves when the fade is complete
 */
...
/**
 * Fade the transition overlay from black
 * @param {number} duration - Duration of the fade in milliseconds
 * @returns {Promise} - Promise that resolves when the fade is complete
 */
...
enableControls() {
    ...
    this.isElevatorActivated = false;
}
...
async transitionToLevel(levelName) {
    ...
    if (levelName === 'corridor') {
        // Unload arcade level
        this.levelManager.unloadArcadeLevel();
    ...
    // Re-enable controls after transition is complete
    this.enableControls();
    
    console.log(`Transitioned to ${levelName} level, controls re-enabled`);
}
